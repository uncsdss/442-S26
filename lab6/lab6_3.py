...
class Lab6_3(ShowBase):
    def __init__(self):
        ...
        self.scoreText = OnscreenText(text="Score: " + str(0.0), pos=(0.0, -0.95), scale=0.13)
        self.reset()
...
def simulationIteration(self, dt):
    if (self.mode!=0):
        ...
        theta_reference = 0.0
        thetadot_reference = 0.0

        #################################
        # compute control input using a simple proportional controller
        ...
        if self.theta>3.14159/2:
            ...
            self.scoreText.setText("Kp=" + str(Kp) + "  Score:" + str(round(self.error_score*180.0/3.14159/t, 1)))
